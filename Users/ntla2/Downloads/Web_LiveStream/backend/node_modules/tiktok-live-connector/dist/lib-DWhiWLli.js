...
var SignatureRateLimitError = class SignatureRateLimitError extends SignAPIError {
	...
	static readHeader(response, name) {
		const value = response.headers[name];
		return typeof value === "string" || typeof value === "number" ? String(value) : void 0;
	}
	static calculateRetryAfter(response) {
		return parseInt(SignatureRateLimitError.readHeader(response, "retry-after") || "0") * 1e3;
	}
...
var WebcastCookieJar = class WebcastCookieJar {
	...
	*/
	async getAnonymousCookieString() {
		const anonymizedStore = {};
		for (let [cookieName, cookieValue] of Object.entries(this.store)) if (!this.webConfig.SESSION_COOKIE_NAMES.includes(cookieName) && cookieName !== this.webConfig.TARGET_IDC_COOKIE_NAME) anonymizedStore[cookieName] = cookieValue;
...
const fetchSignedWebSocketFromEulerRoute = createRoute("fetchSignedWebSocketFromEulerRoute", async ({ routeId, roomId, apiClient, webClient, useMobile, session, authenticateWs, country, cursor, options }) => {
	...
	if (!response.headers["x-set-tt-cookie"]) {
		...
		throw new SignAPIError("Empty Cookies", logId, agentId, `[${routeId}] No cookies received from sign server.`, extra);
	}
	return {
...
var TikTokLiveConnection = class extends EventEmitter {
	...
	*/
	async _connect(roomId) {
		...
		let bundledOptions;
		if (this.options.useMobile && !this.options.authenticateWs) throw new TypeError("Mobile WebSocket requires authenticateWs=true!");
		if (this.options.authenticateWs) {
			...
			if (!cookieBundle && !oauthBundle) throw new TypeError("A cookie or OAuth bundle is required to use the authenticated WebSocket!");
			bundledOptions = {
				authenticateWs: true,
	...
	*/
	async setupWebsocket(wsUrl, wsParams, roomId) {
		const wsClient = this._wsClientInstance = this._wsClientProvider({
			...
			baseUrl: wsUrl,
			wsParams,
			wsHeaders: { Cookie: this.options.authenticateWs ? await this.webClient.cookieJar.getCookieString() : await this.webClient.cookieJar.getAnonymousCookieString() }
		});
		const connectPromise = new Promise((resolve, reject) => {
			...
			const onUnexpectedResponse = (_, res) => {
				clearTimeout(connectTimeout);
				reject(/* @__PURE__ */ new Error(`Error connecting to WebSocket with message: ${res.headers?.["handshake-msg"] || "N/A"}`));
			};
...
//# sourceMappingURL=lib-DWhiWLli.js.map